...
with col1:
    ...
    if kw1:
        ...
        if matched_key:
            ...
            st.markdown(f"<div style='background-color: #2c3e50; padding: 12px; border-radius: 8px; color: white; border-left: 5px solid #3498db;'>💡 最適な基準: <b>{target_sousoku_name}</b></div><br>", unsafe_allow_html=True)
        else:
            st.markdown("<div style='color: white;'>特定の基準指定はありません。（仕様書全体から検索します）</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div style='color: white;'>上にキーワードを入力すると、関連する基準・総則がここに表示されます。</div>", unsafe_allow_html=True)
